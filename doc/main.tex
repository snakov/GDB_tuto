...
\frame(\frametitle{Improve gdb's output} 
  \begin{block}{}
    \begin{itemize}
    ...
    \item User define helpers in the {\bf .gdbinit} file. \\ First add
      this line to your global gdbinit (~/.initgdb):\\ set auto-load
      safe-path # disable safe checks \\ add-auto-load-safe-path
      /path/to/your/working/gdbinit # a bit safer
      \begin{lstlisting}[language=Bash]
        define my_print
        \t set language c  \# tell gdb what language to use
        \t printf ``Here is the first arg: <\%s>'', \$arg0
        \t set \$n = 0  # define local variable
        \t while \$n \!= \$arg1
        \t\t printf ``Second argumnet is not \%d\textbackslash n'', $n
        \t\t \$n = \$n +1
        \t end
        \t printf ``Last argument is \%d\textbackslash n'', \$arg1
        end 
      \end{listing}
    \end{itemize}
  \end{block}
